...
**`update_work_item_links` is per-link fan-out, not bulk.** PATCH `/linkedworkitems/{role}/{tgtProj}/{tgtWI}` has no server-side bulk equivalent, so the tool issues N sequential PATCH calls (~1.5 s apart) — batches above ~30 links exceed the 60 s tool timeout. It accepts a `list[WorkItemLinkUpdateSpec]` for caller convenience; on the first per-link failure the loop halts and the returned `WorkItemLinksUpdateResult` carries `updated=False`, `link_ids` for the successfully-patched prefix, and `failed_link_id` / `failed_reason` for the link that halted the loop. Only `PolarionAuthError` still raises (`PermissionError`) because auth applies globally; 404 / 4xx / 5xx per-link errors are returned in the result. `suspect` is tri-state on update (`None` = leave unchanged) — distinct from `WorkItemLinkSpec.suspect: bool = False` on create — and the `_at_least_one_attribute` validator rejects specs that set neither `suspect` nor `revision` so we never ship an empty PATCH body.
