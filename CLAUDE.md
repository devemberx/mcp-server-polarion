...
**`create_work_item_links` is bulk-only.** One POST on the same `/linkedworkitems` path carries `list[WorkItemLinkSpec]` (`min_length=1`); each spec composes one `linkedworkitems` resource. The returned composite ids `<srcProj>/<srcWI>/<role>/<tgtProj>/<tgtWI>` are the path identifiers for subsequent PATCH / DELETE of the same links. Mixed-success behavior (e.g. one duplicate among valid links) is **not currently characterised** on this server — on any 4xx response, assume nothing was committed and re-query with `list_work_item_links(direction="forward")` before retrying. **Phantom-success on same-role conflict** (verified 2026-05-25 on testdrive): when the source work item is attached to a document, `move_work_item_to_document` auto-creates one outgoing link from the source (see "Work item ↔ document attach / detach" below); posting a NEW link with the SAME role from that source returns 201 with `link_ids` echoing the request, but the new link is NOT persisted. There is no 4xx and `list_work_item_links` confirms only the pre-existing auto-link survives. Recovery: `move_work_item_from_document` → `create_work_item_links` → `move_work_item_to_document`.
