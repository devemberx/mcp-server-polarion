...
- Tool docstrings are the LLM's manual — Google-style with Args/Returns/Raises. Keep return-field bullets in sync with the Pydantic model. **Only the prose above `Args:` ships to clients** — FastMCP strips `Args:`/`Returns:`/`Raises:` from the description it sends, so those sections are dev-facing only (they do not steer the LLM and do not count against the load context). Keep the prose tight: it is the single largest always-loaded payload.
...
- Field descriptions stay one line; skip when name + type say everything. Both model `Field(description=...)` (shipped via `outputSchema`) and tool-parameter `Field(description=...)` (shipped via `inputSchema`) count against the load context, so **omit the description when the field name is unambiguous** (`id`, `title`, `status`, `page_size`, …) and never restate constraints the schema already encodes (`ge`/`le`/`default`/`min_length`/`Literal`/type). Two corollaries: (1) **`e.g.` examples are input-only** — they earn their keep on input specs / tool params (`WorkItemCreateSpec.type`, `priority`, …) where they steer the LLM's choice, but on output / result models the LLM reads the value rather than picking it, so drop them (`WorkItemSummary.type`, `DocumentDetail.status`, …); (2) **no repeated result-model boilerplate** — don't restate dry-run / payload-preview semantics on every result model: the adjacent `dry_run` field makes `created/updated/…: "False on dry-run."` self-evident (omit it), and the `payload_preview` `None`-after-real-op contract lives in the write-tool prose, not per field. Keep descriptions that carry non-obvious semantics: units, empty-conditions, round-trip / read-only contracts, tri-state `None`, ordering guarantees. Cross-model invariant: `tests/test_models.py::test_field_descriptions_are_non_empty_when_set` (non-empty *when set*; omission is allowed).
...
- **Lucene**: trailing wildcards OK, leading wildcards 400. `module` and `description` are not indexed — use `query="SQL:(...)"` on `list_work_items` for module-scoped / custom-field / traceability / body searches. The `list_work_items` docstring keeps only the SQL fundamentals (escaping, `C_DESCRIPTION` CLOB caveat, `LIKE`-in-`EXISTS`→`INNER JOIN`) plus a pointer; the full schema + copy-paste recipe gallery lives in the `polarion://guides/sql-query-recipes` MCP resource (`tools/read.py:sql_query_recipes`, fetched on demand so it costs no load context), with the upstream schema in [SQLQueryExamples.pdf](https://testdrive.polarion.com/polarion/sdk/doc/database/SQLQueryExamples.pdf). `LIKE` is rejected inside `EXISTS (SELECT ...)` — keep `LIKE` in the top-level `WHERE` via `INNER JOIN`.
