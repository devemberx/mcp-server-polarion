...
- **Custom fields inline under `attributes`** (no `customFields` container; `@all` tokens dropped). Server fetches `fields[*]=@all` and splits via `STANDARD_*_ATTRIBUTES` allowlists. Polarion does NOT validate custom-field ids — unknown keys persist as silent ghosts; wrong-type values 400.
  - Guarded writes: `guard_work_item_custom_field_keys` validates both `create_work_items` and `update_work_item.custom_fields` against the type's real schema — a `(project, type)` MIN-per-key SQL sample (`GROUP BY cf.c_name`, one representative item per distinct key, `fields=@all` → complete schema in one request), cached as the sole source of truth. SQL rejection degrades to a bounded Lucene `type:X` sample. A type whose items populate no custom field yields an empty schema and fails closed. A would-be-unknown key triggers one fresh re-fetch (admin-added field / sparse-sample miss) before rejecting. The heavy count-ordered window SQL (`MAX(COUNT(...)) OVER ()`) is deliberately avoided — full-partition per-item aggregation, unsafe at ≤3 req/s; MIN-per-key groups only on the indexed `c_name`. **Documents** mirror this on the **document-type** axis: `guard_document_custom_field_keys` validates both `create_document` and `update_document.custom_fields` against a `(project, document_type)` schema cache. The `module` table is unreachable via REST SQL, so the sample is one project-wide `GET /projects/{p}/documents?fields[documents]=@all` (paged) — every document's type + inline customs in one request, unioned per type (all types stored from one fetch). `update_document` resolves the doc's current type via a narrow GET (or the new `type` when changing it); `create_document` uses its `type` arg. Same fail-closed-on-empty + bypass-retry as work items.
