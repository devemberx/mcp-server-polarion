...
- Error mapping: `PolarionNotFoundError`→`ValueError`, `PolarionAuthError`→`PermissionError`, `PolarionError`→`RuntimeError`; user-fixable status allowed narrower map (attachment 409 dup fileName→`ValueError`). Auth map go through `_shared/errors.py` `auth_error` only (gate `test_error_message_style.py`) — 403 mean token scope OR workflow lock, so server detail ship verbatim; tool with document coordinate in args add `document_status_hint` (failure-path GET, swallow own errors) and drop token wording.
