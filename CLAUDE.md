...
**`/backlinkedworkitems` is not supported on this server.** `list_work_item_links` returns one direction per call: forward links use `/projects/{p}/workitems/{work_item}/linkedworkitems`; back links fall back to a `query=linkedWorkItems:{wi}` search that does not expose the originating role, so back items return with `role=None`. Call twice when both directions are needed. `create_work_item_links` (bulk) adds outgoing links via POST on the same path; `delete_work_item_links` (bulk) removes them via DELETE with a JSON:API body listing composite ids `<srcProj>/<srcWI>/<role>/<tgtProj>/<tgtWI>`. Both tools accept `list[WorkItemLinkSpec]` / `list[WorkItemLinkRef]` with `min_length=1`; the delete tool re-assembles the composite id from `role + target_work_item_id` so the LLM never handles the raw 5-segment string. **Delete is silently idempotent at the body level** (verified 2026-05-22 against testdrive): refs whose composite id does not match an existing link are ignored and the request returns 204; refs that do match are deleted in the same call. The only 404 the tool surfaces is path-level (source WI missing). A malformed composite id in the body returns 400, but the tool layer constructs valid ids from structured refs so this is unreachable in normal use. Mixed-success on `create_work_item_links` is still uncharacterised.
