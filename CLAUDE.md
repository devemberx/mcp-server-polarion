...
- Testrecords: PATCH batch atomic (one bad id 400 whole batch, "was not found" = 400 not 404); server validate neither `result` nor `defect` target (204 ghost) — guard pre-write via `testing/test-result` enum + workitem existence; partial PATCH safe (omitted attrs preserved); REST auto-fill nothing (`executed`/`duration`/`testCaseRevision` UI-only); `text/plain` comment stored as `text/html`; run type may require e-signature → record write 403 portal-only remedy, REST cannot supply — surface Polarion detail in error, token hint alone mislead.
