...
- Server limits: ≤3 req/s, no concurrency. Client serializes via lock + paces every request to ≤3 req/s (start-based min-interval, so slow requests add no extra wait); writes also add a 1.5s post-delay; retries 429/5xx. Pacing is host-global: `GlobalPacer` (`core/global_pace.py`) holds a shared `filelock` across each request so all local server processes share one ≤3 req/s budget; disabled when `min_interval=0` or `pace_lock_path=None`.
